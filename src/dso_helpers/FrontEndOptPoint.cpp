...
PointHessian *
FrontEnd::optimizeImmaturePoint(ImmaturePoint *point, int minObs,
  ...
  assert(nres == ((int)frame_hessians_.size()) - 1);
  ...
  for (int i = 0; i < nres; i++) {
    lastEnergy += point->linearizeResidual(&h_calib_, 1000, residuals + i,
  ...
  for (int iteration = 0; iteration < setting_GNItsOnPointActivation;
    ...
    float newEnergy = 0;
    for (int i = 0; i < nres; i++)
      newEnergy += point->linearizeResidual(&h_calib_, 1, residuals + i, newHdd,
                                            newbd, newIdepth);
  ...
  PointHessian *p = new PointHessian(point, &h_calib_);
  ...
  for (int i = 0; i < nres; i++)
    if (residuals[i].state_state == ResState::IN) {
      ...
      if (r->target == frame_hessians_.back()) {
        p->lastResiduals[0].first = r;
        p->lastResiduals[0].second = ResState::IN;
      } else if (r->target ==
                 (frame_hessians_.size() < 2
                      ? 0
                      : frame_hessians_[frame_hessians_.size() - 2])) {
